...
void HorzBlurCS(int3 groupThreadID : SV_GroupThreadID, int3 dispatchThreadID : SV_DispatchThreadID)
{
    ...
    float sumOfIntensityWeight = 0;
    
    // 가로x 블러를 먹인다.
    for (int i = -gBlurRadius; i <= gBlurRadius; i++)
    {
        // 이거는 일단 위치에 따른 가우시안 가중치를 냅다 넣는, Spatial Blur이다.
        int k = groupThreadID.x + gBlurRadius + i;
        float4 spatialColor = weights[i + gBlurRadius] * gCache[k];
                
        // 이거는 밝기 차이가 너무 심하면, TargetPixel에 영향을 줄이는 Intensity Blur(?) 이다.
        float4 TargetPixel = gCache[groupThreadID.x];
        float intensityGap = length(TargetPixel - gCache[k]) / length(float4(1.f, 1.f, 1.f, 1.f));
        float intensityWeight = (1 - intensityGap);
        
        sumOfIntensityWeight += intensityWeight;
        
        blurColor += spatialColor * intensityWeight;
    }
...
void VertBlurCS(int3 groupThreadID : SV_GroupThreadID, int3 dispatchThreadID : SV_DispatchThreadID)
{
    ...
    for (int i = -gBlurRadius; i <= gBlurRadius; i++)
    {
        ...
        // 이거는 밝기 차이가 너무 심하면, TargetPixel에 영향을 줄이는 Intensity Blur(?) 이다.
        float4 TargetPixel = gCache[groupThreadID.y];
        float intensityGap = length(TargetPixel - gCache[k]) / length(float4(1.f, 1.f, 1.f, 1.f));
        float intensityWeight = (1 - intensityGap);
