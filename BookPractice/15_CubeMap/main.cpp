...
public:
	...
	CubeMapApp(const CubeMapApp& _other) = delete;
	CubeMapApp& operator=(const CubeMapApp& _other) = delete;
...
void CubeMapApp::Update(const GameTimer& _gt)
{
...
void CubeMapApp::OnMouseDown(WPARAM _btnState, int _x, int _y)
{
...
void CubeMapApp::OnMouseUp(WPARAM _btnState, int _x, int _y)
{
	// 마우스를 놓는다.
	ReleaseCapture();
}

void CubeMapApp::OnMouseMove(WPARAM _btnState, int _x, int _y)
{
...
void CubeMapApp::OnKeyboardInput(const GameTimer _gt)
{
...
void CubeMapApp::UpdateObjectCBs(const GameTimer& _gt)
{
...
void CubeMapApp::UpdateMaterialCBs(const GameTimer& _gt)
{
...
void CubeMapApp::UpdateMainPassCB(const GameTimer& _gt)
{
...
void CubeMapApp::AnimateMaterials(const GameTimer& _gt)
{
}
...
void CubeMapApp::BuildStageGeometry()
{
...
float CubeMapApp::GetHillsHeight(float _x, float _z) const
{
	return 0.3f * (_z * sinf(0.1f * _x) + _x * cosf(0.1f * _z));
}

XMFLOAT3 CubeMapApp::GetHillsNormal(float _x, float _z) const
{
	// n = (-df/dx, 1, -df/dz)
	// 이게 왜그러냐면
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 10> CubeMapApp::GetStaticSamplers()
{
